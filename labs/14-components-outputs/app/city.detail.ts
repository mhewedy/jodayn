...
@Component({
	...
	<h2>City details
		...
		<img src="img/{{ city.name}}.jpg" alt="Picture of {{ city.name }}" class="img-responsive"/>
	`
})
...
export class CityDetail {
	...
	rate(num: number): void {
		console.log('rating on ', this.city.name, ': ', num);
		this.rating.emit(num);
	}

	ngOnChanges() {
		if (this.city) {
			console.log('in Changes: new city requested');
			this.isFavorite = this.city.favorite;
		}
	}

	// This city is a favorite. Or not!
	favorite() {
